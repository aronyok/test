 ...
 */
public class FragArtistAlbums extends Fragment implements SearchView.OnQueryTextListener {
    ...
    private void loadData() {
        if (InternetConnectivity.isConnectedToInternet(getActivity())) {
            ...
            params.put("start","0");
            params.put("sync_time","2016-04-04");
            client.post("http://52.89.156.64/index.php/api/get_artist_wise_album", params, new JsonHttpResponseHandler() {
                ...
                public void onSuccess(int statusCode, Header[] headers, JSONObject response) {
                    ...
                    Log.e("ALBUM","id "+artistId);

                    if (CommunicationLayer.parseArtistAlbumPost(response) == 1) {
                        //prepareDis();
                            //Globals.albumslist=SparkleApp.getInstance().getAlbumList();
                            prepareDis();
    ...
    private void prepareDis() {
        DbManager myDb = new DbManager(getContext());
        artist = new MoArtist();
        artist = myDb.getArtist(artistId);
        albums = myDb.getAlbumsByArtist(artistId);
        myDb.close();
        adapter.setData(albums);
        //SparkleApp.getInstance().setAlbumList(albums);
    }
