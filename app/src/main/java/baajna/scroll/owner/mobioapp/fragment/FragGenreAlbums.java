 ...
 */
public class FragGenreAlbums extends Fragment implements SearchView.OnQueryTextListener {
    ...
    public static FragGenreAlbums getInstance(int genreId) {
        FragGenreAlbums fragAlbum = new FragGenreAlbums();
    ...
    private void loadData() {
        if (InternetConnectivity.isConnectedToInternet(getActivity())) {
            ...
            params.add("genre_id","11");
            params.add("start","0");
            params.add("sync_time","2016-04-04");
            client.post("http://52.89.156.64/index.php/api/get_genre_wise_album", params, new JsonHttpResponseHandler() {
                @Override
                public void onSuccess(int statusCode, Header[] headers, JSONObject response) {
                    super.onSuccess(statusCode, headers, response);
                    Log.e("ALBUM Genre", response.toString());

                    if (CommunicationLayer.parseGenreWiseAlbumPost(response) == 1) {
                        prepareDis();
    ...
    private void prepareDis() {
        ...
        //artist = new MoArtist();
        //artist = myDb.getArtist(artistId);
        albums = myDb.getAlbumsByGenre(genreId);
